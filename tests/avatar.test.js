...
test('Check if avatar exists', async ({ page }) => {
	await page.waitForSelector('#wapuugotchi__avatar .wapuugotchi__svg svg', { state: 'visible' });
});

test('Check if bubble is displayed', async ({ page }) => {
	await page.waitForSelector('#wapuugotchi__avatar .wapuugotchi__bubble', { state: 'visible' });
});
