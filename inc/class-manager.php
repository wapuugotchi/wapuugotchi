...
class Manager {
	...
	public function init($hook_suffix) {
		...
		if ( empty( get_user_meta( get_current_user_id(), 'wapuugotchi_config', true ) ) ) {
			update_user_meta( get_current_user_id(), 'wapuugotchi_config',
			...
			sprintf(
				"wp.data.dispatch('wapuugotchi/wapuugotchi').__initialize(%s)", json_encode(
					[
						...
						'wapuu'        => json_decode( get_user_meta( get_current_user_id(), 'wapuugotchi_config', true ) ),
