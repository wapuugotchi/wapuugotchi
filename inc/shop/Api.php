...
class Api {
	...
	public function update_purchases( $req ) {
		...
		if ( ! isset( $req['item'] ) || ! isset( $req['item']['key'], $req['item']['category'] ) ) {
			return rest_ensure_response( new \WP_REST_Response( array( 'status'  => '404',
		...
		$payed = BalanceHandler::pay_item( $item );
		if ( ! $payed ) {
			return rest_ensure_response( new \WP_REST_Response( array( 'status'  => '404',
		...
		return rest_ensure_response( new \WP_REST_Response( array( 'status'  => '200',
